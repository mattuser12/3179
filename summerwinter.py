...
players_count = df.groupby(['Year', 'Season', 'Sex', 'City']).size().reset_index(name='Player_Count')

winter_olympics = players_count[players_count['Season'] == 'Winter']
summer_olympics = players_count[players_count['Season'] == 'Summer']

winter_olympics.to_csv("Winter Olympics.csv", index = True)
summer_olympics.to_csv("Summer Olympics.csv", index = True)